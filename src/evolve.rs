...
pub fn normalize<T: Metric>(
    ...
    let p0_a = (-b * b - f64::sqrt(delta)) / (2.0 * a);
    let p0_b = (-b * b + f64::sqrt(delta)) / (2.0 * a);
...
pub fn rk4_step<T: Metric>(state: &mut StateVector, metric: &T, h: f64) {
    ...
    state.p = state.p + h * k1.p / 2.0;
    state.x = state.x + h * k1.x / 2.0;

    let k2 = rhs(state, metric);
    state.p = state.p + h * k2.p / 2.0;
    state.x = state.x + h * k2.x / 2.0;

    let k3 = rhs(state, metric);
    state.p = state.p + h * k3.p;
    state.x = state.x + h * k3.x;

    state.p = state.p + h * (k1.p / 6.0 + k2.p / 3.0 + k3.p / 3.0 + k1.p / 6.0);
    state.x = state.x + h * (k1.x / 6.0 + k2.x / 3.0 + k3.x / 3.0 + k1.x / 6.0);
}
